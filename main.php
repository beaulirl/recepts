...
if (isset($_POST['msg'])) {
    ...
    $form_string = FormString($first). "</br>" .FormString($second). "</br>" .FormString($salad). "</br>" .FormString($desert). "</br>". $sum;
    $GLOBALS['form_string'] = $form_string;
}
...
if (isset($_SESSION["mail"])) {
	$b->SetTableName(ConfigurationManager::get('db_tbname'));	
    $recepts = new Dish($b);
    $recepts_array = $recepts->getUserRecepts($_SESSION["mail"]);
    $GLOBALS['recepts_array'] = $recepts_array;
}